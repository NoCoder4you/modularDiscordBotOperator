...
class DenyByDefaultAuthorizer:
    KNOWN_PERMISSIONS = frozenset(
        {"bots.view", "bots.start", "bots.stop", "bots.restart", "operations.view"}
    )
...
class SlidingWindowLimiter:
    ...
    def __init__(self, limit: int, window_seconds: float, monotonic: Callable[[], float] = time.monotonic):
        self.limit = limit
...
class ManagementDependencies:
    ...
    auth_limiter: SlidingWindowLimiter = field(
...
SAFE_SUPERVISOR_ERRORS = {
    ...
    "operation_in_progress": (409, "operation_in_progress", "A lifecycle operation is in progress."),
...
def install_management_api(app: FastAPI, deps: ManagementDependencies) -> None:
        ...
        if request.url.path.startswith("/api/management/"):
            ...
            if length:
                try:
                    ...
                    too_large = True
                if too_large:
                    response = error_response(request, 413, "invalid_request", "Request is too large.")
                    return response
    ...
    def require(request: Request, actor: Principal, permission: str, bot_id: str | None = None) -> None:
        if not deps.authorizer.can(actor, permission, bot_id=bot_id):
            emit("management.permission_denied", request, actor=actor.principal_id, action=permission, bot_id=bot_id, result="denied")
            raise ApiFailure(403, "permission_denied", "Permission is denied.")
    ...
    async def bots(request: Request, actor: Principal = Depends(principal)) -> list[BotView]:
        ...
        return [bot_view(bot) for bot in deps.supervisor.list_bots() if deps.authorizer.can(actor, "bots.view", bot_id=bot.bot_id)]

    @app.get("/api/management/v1/bots/{bot_id}/health", response_model=HealthView)
    async def bot_health(bot_id: str, request: Request, actor: Principal = Depends(principal)) -> HealthView:
    ...
    async def lifecycle(bot_id: str, action: Literal["start", "stop", "restart"], request: Request, actor: Principal = Depends(principal)) -> LifecycleView:
        bot = resolve_bot(bot_id)
        ...
        try:
            ...
            status, code, message = SAFE_SUPERVISOR_ERRORS.get(exc.code, (502, "lifecycle_failed", "Lifecycle operation failed."))
            emit(f"bot.{action}.requested", request, actor=actor.principal_id, action=action, bot_id=bot.bot_id, result=code)
            logger.error("management lifecycle failure request_id=%s code=%s", request.state.request_id, exc.code)
            raise ApiFailure(status, code, message) from None
        emit(f"bot.{action}.requested", request, actor=actor.principal_id, action=action, bot_id=bot.bot_id, operation_id=result.operation.operation_id, result="accepted")
        return lifecycle_view(result)

    @app.get("/api/management/v1/operations/{operation_id}", response_model=OperationView)
    async def operation(operation_id: str, request: Request, actor: Principal = Depends(principal)) -> OperationView:
        if len(operation_id) > 64 or not OPERATION_ID_PATTERN.fullmatch(operation_id):
            raise ApiFailure(404, "operation_not_found", "Operation was not found.")
    ...
    async def internal_error(request: Request, _exc: Exception) -> JSONResponse:
        ...
        logger.error("management request failed request_id=%s code=internal_error", request.state.request_id)
...
def bot_view(bot: RegisteredBot) -> BotView:
    return BotView(bot_id=bot.bot_id, display_name=bot.display_name, enabled=bot.enabled, startup_policy=bot.startup_policy)


def operation_view(record: OperationRecord) -> OperationView:
    return OperationView(operation_id=record.operation_id, bot_id=record.bot_id, action=record.action.value, status=record.status.value, requested_at=record.requested_at, started_at=record.started_at, completed_at=record.completed_at, error_code=record.error_category)


def lifecycle_view(result: LifecycleResult) -> LifecycleView:
    return LifecycleView(operation=operation_view(result.operation), previous_state=result.previous_state.value, current_state=result.current_state.value)


def error_response(request: Request, status: int, code: str, message: str) -> JSONResponse:
    request_id = getattr(request.state, "request_id", str(uuid.uuid4()))
    return JSONResponse(status_code=status, content={"error": {"code": code, "message": message, "request_id": request_id}}, headers={"X-Request-ID": request_id})
