...
try:
    ...
    from rpa_admin.cogs.UserNameChange import UsernameChangeCog, UsernameChangeRequestView
...
class UsernameChangeCogTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_reload_autoroles_cog_loads_extension_when_not_already_loaded(self) -> None:
        bot = SimpleNamespace(
            reload_extension=AsyncMock(side_effect=commands.ExtensionNotLoaded("rpa_admin.cogs.ServerAutoRolesRPA")),
        ...
        bot.load_extension.assert_awaited_once_with("rpa_admin.cogs.ServerAutoRolesRPA")
