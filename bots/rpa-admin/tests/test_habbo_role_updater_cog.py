...
try:
    from rpa_admin.cogs.ServerAutoRolesRPA import HabboApiError, HabboRoleUpdaterCog
except (ModuleNotFoundError, ImportError):  # pragma: no cover - environment-dependent test skip
...
class HabboRoleUpdaterCogEmbedTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_member_join_reapplies_saved_roles_nickname_and_verification_log(self) -> None:
        ...
        with unittest.mock.patch("rpa_admin.cogs.ServerAutoRolesRPA.fetch_habbo_profile", return_value={"name": "Siren"}):
    ...
    async def test_member_join_skips_resync_for_restricted_verified_user(self) -> None:
        ...
        with unittest.mock.patch("rpa_admin.cogs.ServerAutoRolesRPA.fetch_habbo_profile", return_value={"name": "Danger"}):
            await cog.on_member_join(member)
    ...
    async def test_sync_all_verified_users_stops_after_rate_limit_error(self) -> None:
        ...
        with unittest.mock.patch(
            "rpa_admin.cogs.ServerAutoRolesRPA.fetch_habbo_profile",
    ...
    async def test_sync_all_verified_users_applies_manual_delay_between_profile_fetches(self) -> None:
        ...
        with (
            ...
            unittest.mock.patch("rpa_admin.cogs.ServerAutoRolesRPA.asyncio.sleep", new_callable=AsyncMock) as sleep_mock,
        ):
...
class AutoRoleUpdaterRateLimitTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_updater_uses_ten_minute_cycle_and_shared_ip_pacing(self) -> None:
        from rpa_admin.cogs.ServerAutoRolesRPA import AutoRoleUpdater
    ...
    async def test_request_limiter_waits_for_four_second_spacing(self) -> None:
        ...
        with (
            unittest.mock.patch("rpa_admin.cogs.ServerAutoRolesRPA.asyncio.get_running_loop", return_value=loop),
            unittest.mock.patch("rpa_admin.cogs.ServerAutoRolesRPA.asyncio.sleep", new_callable=AsyncMock) as sleep_mock,
        ):
