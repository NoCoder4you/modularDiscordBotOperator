...
LOG_FILE = str(RPAAdminConfig.from_env(require_token=False).paths.bot_logs("rpa-admin") / "bot_errors.log")
...
async def load_cogs():
    ...
    for extension in extensions:
        try:
            await bot.load_extension(f"rpa_admin.cogs.{extension}")
            print(f"[LOADED] - rpa_admin.cogs.{extension}")
        except Exception as e:
            logger.error(f"Failed to load cog rpa_admin.cogs.{extension}: {e}")
            print(f"--- !!! [FAILED] !!! --- - rpa_admin.cogs.{extension}: {e}")
...
async def load(ctx, extension: str):
    try:
        ext = extension if extension.startswith("rpa_admin.cogs.") else f"rpa_admin.cogs.{extension}"
...
async def reload_all(ctx):
    try:
        ...
        for extension in extensions:
            ...
            await bot.reload_extension(f"rpa_admin.cogs.{extension}")
...
async def on_ready():
    ...
    if not getattr(bot, "_rpa_cogs_loaded", False):
        try:
            await load_cogs()
            bot._rpa_cogs_loaded = True
        except Exception as e:
...
def main() -> None:
    """Run RPA Admin as its own Discord process."""
    config = RPAAdminConfig.from_env()
    logger.info("Starting rpa-admin")
    try:
        bot.run(config.token, log_handler=None)
    finally:
        logger.info("Stopped rpa-admin")
