...
logger = logging.getLogger("bot.cda-admin")
...
def resolve_extension_name(extension: str) -> str:
    if "." in extension:
        return extension
    return f"cda_admin.cogs.{extension}"
...
def load_statuses(file_path=None):
    if file_path is None:
        file_path = Path(__file__).with_name("defaults") / "statuses.txt"
    try:
...
def main() -> None:
    """Validate configuration and run this bot as an independent process."""
    config = CDAAdminConfig.from_env()
    global logger
    logger = configure_logging("cda-admin", config.log_level)
    logger.info("Starting CDA Admin")
    try:
        bot.run(config.token, log_handler=None)
    finally:
        logger.info("CDA Admin stopped")
