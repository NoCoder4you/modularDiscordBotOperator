...
LOGGER = configure_logging("unbot", _platform_config.log_level)
...
COGS_DIR = str(Path(BASE_DIR) / "cogs")
...
async def load_cogs():
    ...
    for extension in extensions:
        try:
            await bot.load_extension(f"unbot.cogs.{extension}")
            LOGGER.info("Loaded cog unbot.cogs.%s", extension)
        except Exception as e:
            LOGGER.exception("Failed to load cog unbot.cogs.%s", extension)
            print(f"--- !!! [FAILED] !!! --- - unbot.cogs.{extension}: {e}")
...
async def load(ctx, extension: str):
    try:
        ext = extension if extension.startswith("unbot.cogs.") else f"unbot.cogs.{extension}"
...
async def reload_all(ctx):
    try:
        ...
        for extension in extensions:
            ...
            await bot.reload_extension(f"unbot.cogs.{extension}")
...
def load_statuses(file_path=None):
    file_path = file_path or str(Path(BASE_DIR) / "defaults" / "statuses.txt")
    try:
...
@bot.event
async def on_ready():
    LOGGER.info("Discord READY as %s", bot.user.name)
...
async def prepare_bot() -> None:
    """Load extensions once before connecting to Discord."""
    await load_cogs()


def run() -> None:
    """Run UNBOT as its own Discord process."""
    config = UnbotConfig.from_env()
    ...
    LOGGER.info("Starting UNBOT")
    try:
        bot.run(config.token, log_handler=None)
    finally:
        LOGGER.info("UNBOT stopped")
