...
from .paths import SERVER_CONFIG_PATH, VOID_DATA_FILE
from cda_pay.timekeeping import london_now
...
class PayVoid(commands.Cog):
    ...
    async def payvoid(self, interaction: discord.Interaction, username: str):
        ...
        result = record_void(rec, london_now().replace(tzinfo=None))
        self.save()
        ...
        if result.banned:
            ...
            embed = discord.Embed(
                ...
                description=(
                    f"**User:** `{label}`\n"
                    f"**Until:** <t:{int(result.ban_until.timestamp())}:f>"
                ),
