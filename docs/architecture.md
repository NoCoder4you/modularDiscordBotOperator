...
5. **Isolated runtime roots.** `RuntimePaths` builds only under `data/<validated-bot-id>` and rejects paths which resolve outside that root. Data paths are authorized capabilities; the atomic writer traverses them with POSIX directory descriptors and no-follow semantics, preventing symlink swaps between validation and use. Source trees contain no runtime data.
...
7. **Logging to stdout first.** Bot-scoped records use unambiguous UTC timestamps and suit supervisor collection without duplicate handlers. Rotation belongs at the deployment/supervisor layer; secret values are never accepted by logging setup.
8. **Atomic durable JSON.** Shared writes use a same-directory temporary file, flush/fsync, atomic replace, and POSIX directory fsyncs for the rename and any newly created parent hierarchy. Callers must first obtain an authorized bot-scoped path. Cross-process locking and schema-specific concurrency remain future concerns.
